...
interface LeftSidebarControlsProps {
  ...
  // Adicione aqui quaisquer outras props que LeftSidebarControls precise
}

const LeftSidebarControls: React.FC<LeftSidebarControlsProps> = ({ onGenerate, currentParams }) => {
  ...
  // State for figure selection
  const [selectedFigures, setSelectedFigures] = useState({
    geometric: ['circle', 'square', 'triangle', 'rectangle', 'ellipse', 'polygon', 'crescent', 'arc'],
    organic: ['blob', 'wave', 'curve', 'swirl', 'loop', 'spiral']
  });
  ...
  const applyPreset = (presetColors: string[], themeIndex: number) => {
  ...
  const handleFigureToggle = (type: 'geometric' | 'organic', figure: string) => {
    setSelectedFigures(prev => ({
      ...prev,
      [type]: prev[type].includes(figure)
        ? prev[type].filter(f => f !== figure)
        : [...prev[type], figure]
    }));
  };

  const selectAllFigures = (type: 'geometric' | 'organic') => {
    if (type === 'geometric') {
      setSelectedFigures(prev => ({
        ...prev,
        geometric: ['circle', 'square', 'triangle', 'rectangle', 'ellipse', 'polygon', 'crescent', 'arc']
  ...
  const selectNoFigures = (type: 'geometric' | 'organic') => {
    setSelectedFigures(prev => ({
      ...prev,
      [type]: []
    }));
  ...
  const handleGenerateClick = () => {
    try {
      const finalParams: GenerationParams = {
        ...
        // uploadedImage e imageTransform serão passados via props de App.tsx
        uploadedImage: currentParams.uploadedImage,
        imageTransform: currentParams.imageTransform
      };

      // Store selected figures in a custom property for the generation process
      (finalParams as any).selectedFigures = selectedFigures;

      onGenerate(finalParams);
    } catch (error) {
      console.error('Erro no LeftSidebarControls handleGenerate:', error);
    }
  };

  return (
    <div className="bg-white p-6 border border-zinc-200 rounded-sm shadow-sm">
      <h3 className="serif-font text-xl mb-4">Controles Básicos</h3>
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-2">
          {PRESET_THEMES.map((theme, index) => (
            <button
              key={index}
              onClick={() => applyPreset(theme.colors, index)}
      ...
      <div className="mb-6">
        ...
        <div className="flex justify-between mb-2">
          <button
            onClick={() => selectAllFigures('geometric')}
            className="text-xs px-2 py-1 bg-zinc-100 hover:bg-zinc-200 rounded"
          >
            Todas
          </button>
          <button
            onClick={() => selectNoFigures('geometric')}
            className="text-xs px-2 py-1 bg-zinc-100 hover:bg-zinc-200 rounded"
          >
            Nenhuma
          </button>
        ...
        <div className="grid grid-cols-4 gap-2">
          {['circle', 'square', 'triangle', 'rectangle', 'ellipse', 'polygon', 'crescent', 'arc'].map((figure) => (
            <button
              key={`geom-${figure}`}
              onClick={() => handleFigureToggle('geometric', figure)}
              className={`py-1 text-xs rounded border ${
                selectedFigures.geometric.includes(figure)
      ...
      <div className="mb-6">
        ...
        <div className="flex justify-between mb-2">
          <button
            onClick={() => selectAllFigures('organic')}
            className="text-xs px-2 py-1 bg-zinc-100 hover:bg-zinc-200 rounded"
          >
            Todas
          </button>
          <button
            onClick={() => selectNoFigures('organic')}
            className="text-xs px-2 py-1 bg-zinc-100 hover:bg-zinc-200 rounded"
          >
            Nenhuma
          </button>
        ...
        <div className="grid grid-cols-3 gap-2">
          {['blob', 'wave', 'curve', 'swirl', 'loop', 'spiral'].map((figure) => (
            <button
              key={`org-${figure}`}
              onClick={() => handleFigureToggle('organic', figure)}
              className={`py-1 text-xs rounded border ${
                selectedFigures.organic.includes(figure)
                  ? 'bg-zinc-900 text-white border-zinc-900'
                  : 'bg-white text-zinc-700 border-zinc-200 hover:border-zinc-400'
              }`}
            >
